function computeWeights(alloc, totPallets, totCargo, config) {
  ...
  return {
    ...
    volPerPallet, palletVolWt,
    hubChargeable, hubChargeNote,
    boxVolWt, cfsChargeable, cfsChargeNote,
  };
}

function computeHubCosts(weights, totPallets, config) {
  const { airFreight, sellPrice, hPack, hFork, hIspm, hLtl, hDocs, hMisc } = config;
  ...
  const hAdditional = sellPrice * weights.extraWt;
  ...
  return { hubExtraAFwt, hubExtraAF, hAdditional, hPacking, hForklift, hIspmCost, hLtl, hDocs, hMisc, hubFixed, hubTotal };
}
...
function computeCfsCosts(weights, totPallets, totBoxes, totCargo, config) {
  ...
  const cL = cLtl, cD = cDocs, cM = cMisc;
  const cfsTotalUSD = cR + cS + cP + cL + cD + cM;
  const cfsFixed_inr = cfsTotalUSD * usdRate;
  const cfsTotal = cfsExtraAF + cfsFixed_inr;

  return { cfsExtraAFwt, cfsExtraAF, cR, cS, cP, cL, cD, cM, cfsTotalUSD, cfsFixed_inr, cfsTotal };
}
...
function computeBreakEven(hubCosts, cfsCosts) {
  ...
  let beRate = null, beValid = false, beMessage = '';
...
function computeSensitivity(hubCosts, cfsCosts, airFreight, sensMin, sensMax) {
  ...
  return rates.map(af => {
    ...
    return { afRate: af, isCurrent: Math.abs(af - airFreight) < 0.01, hubExtraAF, hubFixed: hubCosts.hubFixed, hubTotal, cfsExtraAF, cfsFixed: cfsCosts.cfsFixed_inr, cfsTotal, delta, saving: Math.abs(delta), winner };
  });
}
...
function generateRecommendation(hubCosts, cfsCosts, hubTat, cfsTat) {
  ...
  let text, winner;
  if (hubCheap && hubFast) { text = `Hub + LTL recommended — saves ₹${Math.round(saving)} and is ${tatDiff} hrs faster`; winner = 'hub'; }
  else if (!hubCheap && !hubFast) { text = `CFS + LTL recommended — saves ₹${Math.round(saving)} and is ${tatDiff} hrs faster`; winner = 'cfs'; }
  else if (hubCheap && !hubFast) { text = `Hub cheaper by ₹${Math.round(saving)}, but CFS is ${tatDiff} hrs faster — pick based on priority`; winner = 'hub'; }
  else { text = `CFS cheaper by ₹${Math.round(saving)}, but Hub is ${tatDiff} hrs faster — pick based on priority`; winner = 'cfs'; }
...
function fullAnalysis(input) {
  ...
  const pLcm = palletL * 2.54, pWcm = palletW * 2.54, pHcm = palletH * 2.54;
  ...
  const sensitivity = computeSensitivity(hubCosts, cfsCosts, airFreight, sensMin || 100, sensMax || 700);
  ...
  return {
    ...
    weights, hubCosts, cfsCosts, breakEven, sensitivity, recommendation,
